...
public class _JavsukiParser extends JavParser {
	public String strGetIdUrl = "http://javsuki.org/page/";
	public String parserName = "javsuki";

	private final static Lock _mutex = new ReentrantLock(true);

	public _JavsukiParser() {
		try {
			init(parserName);
		...
		for (int i=1;i<=actDepth;i++) {
			...
			for (Element divContent : divContents) {
				...
				if (matcher.find()) {
					...
					for (Element subA:subAs) {
						if (subA.attr("rel").contains("bookmark")) {
	...
	public String getAttrImage(Document doc) throws IOException {
		...
		for (Element content : contents) {
			if (content.attr("property").contains("og:image")){
				imgSrc = content.attr("content").toString();
				break;
			}			
		}
	...
	public Vector<String> getAttrCast(Document doc) throws IOException {
		...
		for (Element content : contents) {
			String tmpStr = content.html().toString();
	...
	public String getAttrDate(Document doc) throws IOException {
		...
		for (Element content:contents){
			if (content.attr("class").contains("published time")){
