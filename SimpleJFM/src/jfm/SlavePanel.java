...
import jfm.JfmParser.ParseResult;
...
public class SlavePanel extends JPanel {
	...
	public void setInfo(File inFile) {
		String val = inFile.getName();
		String javId = Utils.getFileName(val);

		try {
			if (ParseResult.NotExist != JfmParser.start(javId)) {
				AvEntry avEntry = null;
				avEntry = JfmParser.getInfoByName(javId);
				if (avEntry == null) {
					return;
				}
				
				if (parentFrame.focusFile == null) {
					return;
				}

//				System.out.printf("n1=%s n2=%s\n",parentFrame.focusFile.getName(), val);
				if (!val.matches(parentFrame.focusFile.getName())){
//					System.out.printf("Return! n1=%s n2=%s\n",parentFrame.focusFile.getName(), val);
					return;
				}
	...
	public void changeImage() {
		...
		if (newHeight == 0)
			newHeight = 300;
//		System.out.printf("ratio=%f, w=%d, h=%d\n", scaleRatio, newWidth, newHeight);
